...
describe('Documents CRUD', () => {
  ...
  it('inserts a document', () => {
    mutate(db, 'INSERT INTO documents (case_id, filename, file_path, doc_type, tags, notes, file_size) VALUES (?, ?, ?, ?, ?, ?, ?)', [
      caseId,
      'contract.pdf',
      '/storage/affaires/1/contract.pdf',
      'Contract',
      'important, final',
      'ملاحظات',
      '1.2 MB'
  ...
  it('inserts multiple document types', () => {
    mutate(db, 'INSERT INTO documents (case_id, filename, file_path, doc_type) VALUES (?, ?, ?, ?)', [
      caseId,
      'jugement.pdf',
      '/storage/jugement.pdf',
      'Jugement'
    ]);
  ...
  it('queries documents by case', () => {
    const docs = query(
      db,
      "SELECT d.*, COALESCE(c.case_number, '') as case_number FROM documents d LEFT JOIN cases c ON d.case_id = c.id WHERE d.case_id = ? ORDER BY d.upload_date DESC",
      [caseId]
    );
