...
describe('Global Search — Cases', () => {
  ...
  it('searches by description', () => {
    ...
    const results = query(
      ...
      ORDER BY case_number ASC LIMIT 6`,
      [like]
    );
    assert.ok(results.length >= 1);
  });
...
describe('Global Search — Documents', () => {
  ...
  it('searches by filename', () => {
    const results = query(
      db,
      'SELECT d.id, d.filename, d.doc_type, c.case_number FROM documents d LEFT JOIN cases c ON d.case_id = c.id WHERE d.filename LIKE ?',
      ['%contract%']
    );
    assert.equal(results.length, 1);
...
describe('Global Search — Events', () => {
  ...
  before(async () => {
    ...
    mutate(db, 'INSERT INTO events (case_id, title, type, date, court) VALUES (?, ?, ?, ?, ?)', [
      cId,
      'جلسة المحكمة',
      'hearing',
      '2026-07-15',
...
describe('Global Search — Expenses (payments)', () => {
  ...
  before(async () => {
    ...
    mutate(db, 'INSERT INTO paiements (affaire_id, date, montant, mode_paiement, remarque) VALUES (?, ?, ?, ?, ?)', [
      cId,
      '2026-06-01',
      5000,
      'espèces',
  ...
  it('searches by mode_paiement', () => {
    const results = query(db, 'SELECT p.id, p.montant, c.case_number FROM paiements p JOIN cases c ON p.affaire_id = c.id WHERE p.mode_paiement LIKE ?', [
      '%virement%'
    ]);
    assert.equal(results.length, 1);
  });
...
describe('FTS4 Full-Text Search', () => {
  ...
  before(async () => {
    ...
    mutate(db, 'INSERT INTO cases (case_number, title, client_id, client_name, court, description) VALUES (?, ?, ?, ?, ?, ?)', [
      ...
      'موكل تجريبي',
      'المحكمة التجارية',
      'هذا النص يستخدم للبحث التجريبي السريع'
    ]);
...
describe('Search Index', () => {
  ...
  it('pre-joins entity relationships for search index', () => {
    const docIndex = query(
      db,
      `SELECT d.id, d.filename as text, 'document' as type, c.case_number
