...
describe('Tasks CRUD', () => {
  ...
  it('inserts a task with all fields', () => {
    mutate(db, `INSERT INTO tasks (title, description, priority, status, due_date, tags, assigned_to, progress, time_tracked)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
describe('Task Analytics', () => {
  ...
  it('groups by priority', () => {
    ...
    const map = {};
    byPriority.forEach(r => { map[r.priority] = r.c; });
...
describe('Workflows', () => {
  ...
  it('creates a workflow with steps', () => {
    ...
    mutate(db, "INSERT INTO workflow_steps (workflow_id, title, step_order, due_days, assigned_role) VALUES (?, ?, ?, ?, ?)", [wfId, 'تقديم الطلب', 0, 7, 'المحامي']);
    mutate(db, "INSERT INTO workflow_steps (workflow_id, title, step_order, due_days, assigned_role) VALUES (?, ?, ?, ?, ?)", [wfId, 'مراجعة', 1, 14, 'المساعد']);
  ...
  it('applies workflow to case (creates tasks)', () => {
    ...
    const steps = query(db, "SELECT * FROM workflow_steps WHERE workflow_id = ? ORDER BY step_order", [wfId]);
    steps.forEach(s => {
      mutate(db, "INSERT INTO tasks (title, case_id, status, due_date, workflow_id, priority) VALUES (?, ?, 'todo', date('now', '+' || ? || ' days'), ?, 'medium')",
...
describe('Task Templates', () => {
  ...
  it('creates a template', () => {
    const tasksJson = JSON.stringify([{ title: 'مهمة 1', priority: 'high' }, { title: 'مهمة 2', priority: 'medium' }]);
  ...
  it('applies template to case', () => {
    ...
    JSON.parse(template.tasks_json).forEach(t => {
      mutate(db, "INSERT INTO tasks (title, case_id, status, due_date, priority, template_id) VALUES (?, ?, 'todo', ?, ?, ?)",
        [t.title, caseId, null, t.priority || 'medium', template.id]);
    });
    const tasks = query(db, "SELECT * FROM tasks WHERE case_id = ?", [caseId]);
    assert.equal(tasks.length, 2);
  });
