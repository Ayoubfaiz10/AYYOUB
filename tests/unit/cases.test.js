...
describe('Cases CRUD', () => {
  ...
  it('inserts case with full fields', () => {
    mutate(
      db,
      `INSERT INTO cases (case_number, title, client_name, court, status, description, total_fees, paid_fees, expenses, priority, case_type, created_date, deadline_date)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
describe('Cases — Client Association', () => {
  ...
  it('associates case with client', () => {
    ...
    mutate(db, 'INSERT INTO cases (case_number, title, client_id, client_name) VALUES (?, ?, ?, ?)', ['2025/010', 'قضية موكل', clientId, 'الموكل الأول']);
...
describe('Cases — Queries', () => {
  ...
  before(async () => {
    ...
    mutate(db, 'INSERT INTO cases (case_number, title, client_id, status, total_fees, paid_fees) VALUES (?, ?, ?, ?, ?, ?)', [
      'C001',
      'Case 1',
      clId,
      'active',
  ...
  it('calculates remaining fees', () => {
    const r = query(
      db,
      "SELECT id, total_fees, paid_fees, (COALESCE(total_fees,0) - COALESCE(paid_fees,0)) as remaining FROM cases WHERE case_number = 'C001'"
    );
  ...
  it('aggregates fees per client', () => {
    const clId = query(db, "SELECT id FROM clients WHERE name = 'Test Client'")[0].id;
    const agg = query(db, 'SELECT COUNT(*) as cnt, COALESCE(SUM(paid_fees),0) as paid, COALESCE(SUM(total_fees),0) as fees FROM cases WHERE client_id = ?', [
      clId
    ]);
