...
async function createDb() {
  ...
  db.run('CREATE TABLE IF NOT EXISTS subtasks (id INTEGER PRIMARY KEY AUTOINCREMENT, task_id INTEGER NOT NULL, title TEXT NOT NULL, done INTEGER DEFAULT 0, FOREIGN KEY (task_id) REFERENCES tasks(id) ON DELETE CASCADE)');
  db.run('CREATE TABLE IF NOT EXISTS task_comments (id INTEGER PRIMARY KEY AUTOINCREMENT, task_id INTEGER NOT NULL, author TEXT DEFAULT \'المحامي\', text TEXT NOT NULL, created_at TEXT DEFAULT (datetime(\'now\',\'localtime\')), FOREIGN KEY (task_id) REFERENCES tasks(id) ON DELETE CASCADE)');
  db.run('CREATE TABLE IF NOT EXISTS workflows (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, description TEXT DEFAULT \'\', case_type TEXT DEFAULT \'\', created_at TEXT DEFAULT (datetime(\'now\',\'localtime\')))');
  db.run('CREATE TABLE IF NOT EXISTS workflow_steps (id INTEGER PRIMARY KEY AUTOINCREMENT, workflow_id INTEGER NOT NULL, title TEXT NOT NULL, step_order INTEGER DEFAULT 0, due_days INTEGER DEFAULT 0, assigned_role TEXT DEFAULT \'\', FOREIGN KEY (workflow_id) REFERENCES workflows(id) ON DELETE CASCADE)');
  db.run('CREATE TABLE IF NOT EXISTS task_templates (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, description TEXT DEFAULT \'\', tasks_json TEXT DEFAULT \'[]\', created_at TEXT DEFAULT (datetime(\'now\',\'localtime\')))');
