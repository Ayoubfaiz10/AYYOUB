...
A.filterTasks = function () {
  ...
  if (A.state.taskSearchQuery) {
    ...
    list = list.filter(
      t => (t.title || '').toLowerCase().includes(q) || (t.case_number || '').toLowerCase().includes(q) || (t.tags || '').toLowerCase().includes(q)
    );
  }
  return list;
};
  ...
  A.safeSet(container, esc =>
    displayed.length
      ...
      <div class="task-body">
        ...
        ${
          tags.length
            ? `<div class="task-tags">${tags
                .slice(0, 3)
                .map(tag => `<span class="task-tag">${esc(tag.trim())}</span>`)
                .join('')}</div>`
            : ''
        }
      </div>
      ...
      <div class="task-actions">
        <button class="btn-icon" data-click="task:open:${t.id}" data-click-stop="true"><i class="ri-eye-line"></i></button>
      </div>
    </div>`;
          })
          .join('')
  ...
  cols.forEach(col => {
    ...
    A.safeSet(el, esc =>
      items.length
        ? items
            .map(
              t => `<div class="kanban-card-v8" draggable="true" data-id="${t.id}" data-status="${col}">
      <div class="kanban-card-title">${esc(t.title)}</div>
      <div class="kanban-card-meta">${esc(t.case_number || '')} ${t.due_date ? '· ' + esc(t.due_date) : ''}</div>
    </div>`
            )
            .join('')
        : '<div style="font-size:10px;color:var(--muted-foreground);text-align:center;padding:20px 0;">—</div>'
    );
...
A.renderPriorityView = function () {
  ...
  A.safeSet(
    ...
    esc =>
      ...
      <div class="priority-group-header"><span class="priority-dot" style="background:${g.color};"></span>${esc(g.label)} <span style="font-size:11px;color:var(--muted-foreground);font-weight:normal;">(${items.length})</span></div>
      ${items
        .map(
          t => `<div class="task-card-v8" data-click="task:open:${t.id}" style="padding:var(--spacing-1-5) var(--spacing-2);">
