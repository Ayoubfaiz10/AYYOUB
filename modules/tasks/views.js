...
A._renderTaskCards = function(displayed, container) {
  ...
  }).join('') : '<div style="text-align:center;padding:60px 20px;color:var(--muted-foreground);"><i class="ri-checkbox-line" style="font-size:48px;display:block;margin-bottom:8px;"></i>لا توجد مهام</div>');
};
...
A.renderKanban = function() {
  ...
  cols.forEach(col => {
    ...
    </div>`).join('') : '<div style="font-size:10px;color:var(--muted-foreground);text-align:center;padding:20px 0;">—</div>');
...
A.renderPriorityView = function() {
  ...
  A.safeSet(container, esc => groups.map(g => {
    ...
    return `<div class="priority-group">
      ...
      ${items.map(t => `<div class="task-card-v8" onclick="openTaskDetail(${t.id})" style="padding:var(--spacing-1-5) var(--spacing-2);">
...
A.renderTaskAnalytics = async function() {
  ...
  A.safeSetStatic(container, `<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:var(--spacing-3);margin-bottom:var(--spacing-3);">
