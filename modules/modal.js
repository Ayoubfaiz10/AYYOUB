...
A.showModal = function (title, content, onConfirm) {
  ...
  if (modalDraftCleanup) {
    modalDraftCleanup();
    modalDraftCleanup = null;
  }
  ...
  if (A.AutoSave && content) {
    ...
    inputs.forEach(el => {
      el.addEventListener('input', modalDebounce);
      el.addEventListener('change', modalDebounce);
    });
    modalDraftCleanup = () =>
      inputs.forEach(el => {
        el.removeEventListener('input', modalDebounce);
        el.removeEventListener('change', modalDebounce);
      });
  }
};
...
A.showConfirm = function (message, confirmText, type) {
  return new Promise(resolve => {
    ...
    if (!overlay || !confirmBtn || !cancelBtn || !iconEl || !titleEl || !msgEl) {
      resolve(true);
      return;
    }
    ...
    titleEl.textContent = confirmText === _t('archiveBtn') ? _t('confirmArchive') : _t('confirmDeleteTitle');
...
A.initModal = function () {
  ...
  document.getElementById('modalOverlay')?.addEventListener('click', e => {
    if (e.target === document.getElementById('modalOverlay')) A.hideModal();
  });
