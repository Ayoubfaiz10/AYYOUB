...
A.navigateTo = function(sectionId) {
  ...
  if (!A.canAccess(sectionId)) { A.navigateTo('dashboard'); return; }
  A.state.navItems.forEach(function(n) { n.classList.remove('active'); });
  A.state.sections.forEach(function(s) { s.classList.remove('active'); });
...
A.initNavigation = function() {
  ...
  A.state.navItems.forEach(function(item) {
    item.addEventListener('click', function() { A.navigateTo(item.dataset.section); });
  });
};
