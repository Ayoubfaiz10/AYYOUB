...
const VALID_CHANNELS = [
  'auth:boot', 'auth:hasPassword', 'auth:login', 'auth:hashPassword',
  'auth:getCurrentUser', 'auth:getPermissions', 'auth:getUsers', 'auth:addUser',
  'auth:updateUser', 'auth:deleteUser', 'auth:updateProfile', 'auth:changePassword',
  'auth:setup',   'auth:checkSession', 'auth:checkRemembered', 'auth:logout',
  'auth:getSecurityQuestion', 'auth:checkSecurityAnswer', 'auth:resetPassword', 'auth:resetWithMasterKey',
  ...
  'db:getDashboardStats', 'db:getDashboardExtendedStats', 'db:getDocuments', 'db:getAllDocuments', 'db:uploadDocument',
  ...
  'db:addCommunication', 'db:getClientCommunications', 'db:getAllCommunications',
  'db:selectAndUpload',
...
function s_array(items)     { return { type: 'array', ...(items ? { items } : {}) }; }
...
const IPC_SCHEMAS = {
  ...
  'auth:updateProfile':      { args: [s_object({
    name:             s_opt(s_string()),
    email:            s_opt(s_string()),
    ...
    avatar:           s_opt(s_string())
  })], strict: true },
  'auth:changePassword':     { args: [s_object({
  ...
  'db:updateTask':           { args: [s_integer({ positive: true }), s_object({
  ...
  'db:uploadDocument':       { args: [s_object({
    sourcePath: s_string({ minLength: 1, pattern: /^[^\\\/]+$/ }),
    caseId:    s_integer({ positive: true }),
...
'db:deleteBackup':          { args: [s_string({ minLength: 1, pattern: /^[^.\\\/][^\\\/]*$/ })], strict: true },
  ...
  'db:addLog':                { args: [s_string({ minLength: 1, maxLength: 5000 }), s_string({ maxLength: 5000 })], strict: true },
  ...
  'events:update':            { args: [s_integer({ positive: true }), s_object({
    case_id:    s_opt(s_integer({ nullable: true })),
    client_id:  s_opt(s_integer({ nullable: true })),
    title:      s_opt(s_string()),
    type:       s_opt(s_string()),
  ...
  'license:activate':         { args: [s_object({
