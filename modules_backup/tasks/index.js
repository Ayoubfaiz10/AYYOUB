...
A.loadTasks = async function() {
  ...
  try {
    ...
    if (mainEl) A.showError(mainEl, A.i18n.t('error') + ': ' + A.i18n.t('loading'), () => A.loadTasks());
  }
};
...
A.showTaskForm = async function(editData) {
  ...
  A.showModal(isEdit ? A.i18n.t('edit') + ' ' + A.i18n.t('tasks') : A.i18n.t('add') + ' ' + A.i18n.t('tasks'), `
    <div class="input-group"><label class="input-label">العنوان</label><input type="text" id="fTaskTitle" class="input" value="${esc(editData?editData.title:'')}" placeholder="` + A.i18n.t('add') + ` ` + A.i18n.t('tasks') + `"></div>
    <div class="input-group"><label class="input-label">الوصف</label><textarea id="fTaskDesc" class="input" rows="3" placeholder="` + A.i18n.t('edit') + ` ` + A.i18n.t('tasks') + `">${esc(editData?editData.description||'':'')}</textarea></div>
    <div class="info-grid-2">
      <div class="input-group"><label class="input-label">القضية</label><select id="fTaskCase" class="input"><option value="">-- ` + A.i18n.t('cancel') + ` --</option>${caseOpts}</select></div>
      <div class="input-group"><label class="input-label">مسؤول</label><input type="text" id="fTaskAssigned" class="input" value="${esc(editData?editData.assigned_to||'':'')}" placeholder="` + A.i18n.t('add') + ` ` + A.i18n.t('tasks') + `"></div>
    </div>
    ...
    <div class="input-group"><label class="input-label">الوسوم (مفصولة بفواصل)</label><input type="text" id="fTaskTags" class="input" value="${esc(editData?editData.tags||'':'')}" placeholder="` + A.i18n.t('priority_high') + `، ` + A.i18n.t('priority_critical') + `"></div>
  `, async () => {
    const title = document.getElementById('fTaskTitle').value.trim();
    if (!title) { A.showToast(A.i18n.t('add') + ' ' + A.i18n.t('tasks') + ' ' + A.i18n.t('required'), 'error'); return; }
    const data = {
    ...
    try {
      if (isEdit) { await A.mutate('db:updateTask', editData.id, data); }
      else { await A.mutate('db:addTask', data); }
      A.hideModal(); A.loadTasks();
    } catch (e) { A.logError('showTaskForm', e); A.showToast('فشل حفظ المهمة', 'error'); }
  });
};
...
A.openTaskDetail = async function(taskId) {
  ...
  A.safeSet(document.getElementById('taskDetailBody'), esc => `
  ...
  document.getElementById('taskDetailEdit').onclick = () => { document.getElementById('taskDetailOverlay').style.display='none'; A.showTaskForm(t); };
  document.getElementById('taskDetailDelete').onclick = async () => {
    if (await A.showConfirm('حذف هذه المهمة؟')) { try { await A.mutate('db:deleteTask', taskId); document.getElementById('taskDetailOverlay').style.display='none'; A.loadTasks(); } catch (e) { A.logError('deleteTask', e); A.showToast('فشل حذف المهمة', 'error'); } }
  };
...
window.toggleSubtask = async function(id) {
  try { await A.mutate('db:toggleSubtask', id); } catch (e) { A.logError('toggleSubtask', e); A.showToast('فشل تحديث المهمة الفرعية', 'error'); return; }
...
window.deleteSubtaskItem = async function(id) {
  try { await A.mutate('db:deleteSubtask', id); A.loadTasks(); } catch (e) { A.logError('deleteSubtask', e); A.showToast('فشل حذف المهمة الفرعية', 'error'); }
};

window.addSubtaskTo = async function(taskId) {
  const input = document.getElementById('newSubtaskInput');
  if (!input||!input.value.trim()) return;
  try { await A.mutate('db:addSubtask', { task_id: taskId, title: input.value.trim() }); } catch (e) { A.logError('addSubtask', e); A.showToast('فشل إضافة المهمة الفرعية', 'error'); return; }
  input.value = '';
...
window.addCommentTo = async function(taskId) {
  ...
  if (!input||!input.value.trim()) return;
  try { await A.mutate('db:addComment', { task_id: taskId, text: input.value.trim(), author: 'المحامي' }); } catch (e) { A.logError('addComment', e); A.showToast('فشل إضافة التعليق', 'error'); return; }
  input.value = '';
  A.openTaskDetail(taskId);
};
...
window.applyWorkflow = async function() {
  ...
  if (!caseId || !wfId) { A.showToast('اختر القضية وسير العمل', 'error'); return; }
  try { await A.mutate('db:applyWorkflow', { caseId, workflowId: wfId }); } catch (e) { A.logError('applyWorkflow', e); A.showToast('فشل تطبيق سير العمل', 'error'); return; }
  A.loadTasks(); if (overlay) overlay.style.display='none';
};
...
window.applyTemplate = async function() {
  ...
  if (!caseId || !tmplId) { A.showToast('اختر القضية والقالب', 'error'); return; }
  try { await A.mutate('db:applyTemplate', { caseId, templateId: tmplId }); } catch (e) { A.logError('applyTemplate', e); A.showToast('فشل تطبيق القالب', 'error'); return; }
  A.loadTasks(); if (overlay) overlay.style.display='none';
};
...
window.showNewWorkflowForm = function() {
  ...
  `, async () => {
    ...
    if (!name) { A.showToast('الاسم مطلوب', 'error'); return; }
    const lines = document.getElementById('wfSteps').value.split('\n').filter(Boolean);
    try { await A.mutate('db:addWorkflow', { name, description: document.getElementById('wfDesc').value, case_type: document.getElementById('wfCaseType').value, steps: lines.map(l => ({ title: l })) }); } catch (e) { A.logError('addWorkflow', e); A.showToast('فشل إضافة سير العمل', 'error'); return; }
    A.hideModal(); document.getElementById('showWorkflowsBtn').click();
  });
};

window.showNewTemplateForm = function() {
  A.showModal('قالب مهام جديد', `
  ...
  `, async () => {
    ...
    if (!name) { A.showToast('الاسم مطلوب', 'error'); return; }
    try { await A.mutate('db:addTemplate', { name, description: document.getElementById('tmplDesc').value, tasks_json: document.getElementById('tmplTasks').value }); } catch (e) { A.logError('addTemplate', e); A.showToast('فشل إضافة القالب', 'error'); return; }
    A.hideModal(); document.getElementById('showWorkflowsBtn').click();
  });
};

A.initTasks = function() {
  document.querySelectorAll('#section-tasks .cases-view-toggle .view-btn').forEach(btn => btn.addEventListener('click', () => {
  ...
  document.getElementById('searchTasks')?.addEventListener('input', A.debounce((e) => { A.state.taskSearchQuery = e.target.value; A.renderTaskList(); }, 250));
  document.getElementById('taskStatusFilter')?.addEventListener('change', (e) => { A.state.taskStatusFilter = e.target.value; A.renderTaskList(); });
  ...
  document.getElementById('kanbanBoardV8')?.addEventListener('drop', async (e) => {
    ...
    try { await A.mutate('db:updateTask', taskId, { status: newStatus }); } catch (e) { A.logError('kanbanDropV8', e); A.showToast('فشل تغيير حالة المهمة', 'error'); }
    A.loadTasks();
  });
};
