...
A.loadExpenses = async function() {
  ...
  try {
    ...
    let totalHonoraires = 0, totalPaid = 0, totalExpenses = 0;
    let allPaiements = [];
    for (const c of cases) {
      ...
      paiements.forEach(p => { p.case_number = c.case_number; allPaiements.push(p); });
    ...
    A.safeSet(document.getElementById('expPaymentsBody'), esc => allPaiements.slice(0, 20).length
      ? allPaiements.slice(0, 20).map(p => `<tr><td>${esc(p.date)}</td><td>${esc(p.case_number || '-')}</td><td><strong>${parseFloat(p.montant).toFixed(2)}</strong></td><td>${esc(p.mode_paiement)}</td><td>${esc(p.remarque || '-')}</td></tr>`).join('')
